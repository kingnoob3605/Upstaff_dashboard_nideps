...
window.SupabaseAuth = (function () {
  ...
  const DEFAULT_KEY =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBiYWJxeWRnemdyY2lxemlkdWdkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyNzEyMTMsImV4cCI6MjA5MTg0NzIxM30.-V7A3KVdA2KyrIjH0jz7PpSGfHo91wTllFy_LxMi-Zg";
  ...
  function _config() {
    try {
      ...
      return { supabaseUrl: DEFAULT_URL, supabaseAnonKey: DEFAULT_KEY };
    }
  }
  ...
  async function ensureRecoverySession() {
    ...
    hash.split("&").forEach(function (p) {
      var i = p.indexOf("=");
      if (i > -1)
        params[decodeURIComponent(p.slice(0, i))] = decodeURIComponent(
          p.slice(i + 1),
        );
    });
    if (!params.access_token || !params.refresh_token) {
      throw new Error(
        "Recovery link is missing tokens. Request a new reset email.",
      );
    }
  ...
  async function login(email, password) {
    ...
    if (!client)
      throw new Error(
        "Supabase not configured. Click ⚙ Change settings below.",
      );
  ...
  async function _fetchAndApplyConfig(accessToken) {
    ...
    try {
      var resp = await fetch(
        c.supabaseUrl + "/rest/v1/config?select=key,value",
        {
          headers: {
      ...
      if (!resp.ok) {
        console.error(
          "[SupabaseAuth] Config fetch failed: " +
            resp.status +
            " " +
            resp.statusText,
        );
      ...
      if (!Array.isArray(rows)) {
        console.warn(
          "[SupabaseAuth] Config fetch returned unexpected data:",
          rows,
        );
        return;
      }
      ...
      try {
        ejs = JSON.parse(
          localStorage.getItem("upstaff_emailjs_config") || "{}",
        );
      } catch (_) {}
  ...
  function _readCooldownMap() {
    try {
      return JSON.parse(localStorage.getItem(_resetCooldownKey) || "{}");
    } catch (_) {
      return {};
    }
  }
  async function sendPasswordReset(email) {
    ...
    map[addr] = Date.now();
    try {
      localStorage.setItem(_resetCooldownKey, JSON.stringify(map));
    } catch (_) {}
  }
